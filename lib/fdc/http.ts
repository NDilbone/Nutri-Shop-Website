...
export function mapFdcError(e: unknown): NextResponse {
  if (e instanceof FdcError) {
    if (e.kind === "rate_limited") {
      const headers = e.retryAfter != null ? { "retry-after": String(e.retryAfter) } : undefined;
      return NextResponse.json(
        { error: { code: "UPSTREAM_RATE_LIMITED", message: "Food database is busy; try again later" } },
        { status: 429, headers },
      );
    }
